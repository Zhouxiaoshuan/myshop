...
class MultiSelectFieldListFilter(ListFieldFilter):
    ...
    cache_config = {'enabled': False, 'keys': 'quickfilter_%s', 'timeout': 3600, 'cache': 'default'}
    ...
    def get_cached_choices(self):
        ...
        return c.get(self.cache_config['keys'] % self.field_path)

    def set_cached_choices(self, choices):
        if not self.cache_config['enabled']:
            return
        c = caches(self.cache_config['cache'])
        return c.set(self.cache_config['keys'] % self.field_path, choices)
