...
class GridLayoutPlugin(BaseAdminPlugin):
    ...
    def get_layout(self, l):
        ...
        return dict({'url': self.admin_view.get_query_string({LAYOUT_VAR: item['keys']}), 'selected': False}, **item)
    ...
    def init_request(self, *args, **kwargs):
        ...
        if active:
            ...
            self._current_layout = self.request.GET.get(LAYOUT_VAR, self._active_layouts[0]['keys'])
            for layout in self._active_layouts:
                if self._current_layout == layout['keys']:
