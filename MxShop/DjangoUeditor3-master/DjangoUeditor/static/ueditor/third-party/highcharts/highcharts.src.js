 ...
 * @param {String|Object} prop The property or an abject of keys-value pairs
...
function attr(elem, prop, value) {
	...
	// else if prop is defined, it is a hash of keys/value pairs
...
defaultOptions = {
	...
	tooltip: {
		...
		headerFormat: '<span style="font-size: 10px">{point.keys}</span><br/>',
...
SVGRenderer.prototype = {
	...
	color: function (color, elem, prop) {
		...
		if (gradName) {
			...
			// Build the unique keys to detect whether we need to create a new element (#1282)
			for (n in gradAttr) {
				if (n !== 'id') {
				...
				if (result !== false && value !== null) { // #620
					...
					// x, y, width, height
						...
						if (wrapper.updateClipping) {
							wrapper[key] = value; // the keys is now 'left' or 'top' for 'x' and 'y'
...
Point.prototype = {
	 ...
	 *     this happens if the control keys (Cmd on Mac) was pressed during clicking.
	 */
	...
	firePointEvent: function (eventType, eventArgs, defaultFunction) {
		...
		if (eventType === 'click' && seriesOptions.allowPointSelect) {
			defaultFunction = function (event) {
				// Control keys is for Windows, meta (= Cmd keys) for Mac, Shift for Opera
...
Series.prototype = {
	...
	setStackedPoints: function () {
		...
		for (i = 0; i < yDataLength; i++) {
			...
			// the sign of y and the stack keys. Stacking is also handled for null values (#739)
	...
	tooltipHeaderFormatter: function (point) {
		...
		if (isDateTime && xDateFormat && isNumber(point.key)) {
			headerFormat = headerFormat.replace('{point.keys}', '{point.keys:' + xDateFormat + '}');
		}
